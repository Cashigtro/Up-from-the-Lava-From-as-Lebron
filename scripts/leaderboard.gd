...
func _ready():
    ...
    request(url + "/scores.json", PackedStringArray(), method)
        ...
        if leaderboard != null:
            ...
            if len(leaderboard) != 1:
                ...
                for i in leaderboard_objects:
                    var label = Label.new()
                    print(i)
                    label.text = (str(count+1) + "# " + i + ": " + str(leaderboard[i]["data"]["score"]))
                    if count == 0:
                    ...
                    label.text = (str(count+1) + "# " + i + ": " + get_time(int(leaderboard[i]["data"]['score'])))
...
func populate_leaderboard_old(result, response_code, header, body):
